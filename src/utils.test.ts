...
describe('parseRouteString', () => {
  ...
  const mockAerodromeRepository: AerodromeRepository = {
  ...
  it('should parse ICAO codes correctly', async () => {
    ...
    const result = await parseRouteString(mockAerodromeRepository, mockReportingPoints, 'AD(EHAM);AD(LFPG)');
  ...
  it('should parse reporting points correctly', async () => {
    const result = await parseRouteString(mockAerodromeRepository, mockReportingPoints, 'RP(TEST)');
  ...
  it('should parse waypoints correctly', async () => {
    const result = await parseRouteString(mockAerodromeRepository, mockReportingPoints, 'WP(52.5,13.4)');
